...
| no-lobby                     | Disable lobby button and functionality                              | false   |
...
- **no-lobby**: Boolean value indicating whether to disable the lobby button and functionality. When set to `true`, the lobby button will be hidden and lobby functionality will be disabled. Default is `false`.

To disable lobby functionality, use: `?no-lobby=true` in the launcher URL
